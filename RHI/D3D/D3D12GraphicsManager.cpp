...
HRESULT D3D12GraphicsManager::CreateDescriptorHeaps() {
    ...
    cbvSrvUavHeapDesc.NumDescriptors =
        kFrameCount *
            (2 * kMaxSceneObjectCount)  // 1 perFrame and 1 per DrawBatch
        + kMaxTextureCount;  // + kMaxTextureCount for the SRV(Texture).
    cbvSrvUavHeapDesc.Type  = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    cbvSrvUavHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    if (FAILED(hr = m_pDev->CreateDescriptorHeap(&cbvSrvUavHeapDesc,
                                                 IID_PPV_ARGS(&m_pCbvHeap)))) {
        return hr;
    }

    m_nCbvSrvDescriptorSize = m_pDev->GetDescriptorHandleIncrementSize(
...
HRESULT D3D12GraphicsManager::CreateVertexBuffer(
    ...
    UpdateSubresources<1>(m_pCommandList, pVertexBuffer,
...
HRESULT D3D12GraphicsManager::CreateIndexBuffer(
    ...
    UpdateSubresources<1>(m_pCommandList, pIndexBuffer, pIndexBufferUploadHeap,
                          0, 0, 1, &indexData);
    D3D12_RESOURCE_BARRIER barrier = {};
    barrier.Type                   = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    barrier.Flags                  = D3D12_RESOURCE_BARRIER_FLAG_NONE;
    ...
    if (it == m_TextureIndex.end()) {
        ...
        if (image.bitcount == 24) {
            ...
            for (uint32_t row = 0; row < image.Height; row++) {
                ...
                src =
                    reinterpret_cast<uint8_t*>(image.data) + row * image.pitch;
            ...
            // we do not need to free the old data because the old data is still
            // referenced by the SceneObject g_pMemoryManager->Free(image.data,
            // image.data_size);
            image.data      = data;
        ...
        UpdateSubresources(m_pCommandList, pTextureBuffer, pTextureUploadHeap,
        ...
        srvHandle.ptr =
            m_pCbvHeap->GetCPUDescriptorHandleForHeapStart().ptr +
            (kTextureDescStartIndex + texture_id) * m_nCbvSrvDescriptorSize;
...
HRESULT D3D12GraphicsManager::CreateSamplerBuffer() {
    ...
    m_pDev->CreateSampler(&samplerDesc,
...
HRESULT D3D12GraphicsManager::CreateConstantBuffer() {
    ...
    for (auto i = 0; i < kFrameCount; i++) {
        for (auto j = 0; j < kMaxSceneObjectCount; j++) {
            ...
            cbvDesc.BufferLocation =
                pConstantUploadBuffer->GetGPUVirtualAddress() +
            ...
            cbvDesc.BufferLocation +=
                kSizePerFrameConstantBuffer + j * kSizePerBatchConstantBuffer;
    ...
    hr                    = pConstantUploadBuffer->Map(0, &readRange,
...
HRESULT D3D12GraphicsManager::CreateRootSignature() {
    ...
    D3D12_DESCRIPTOR_RANGE1 ranges[3] = {
        {D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 2, 0, 0,
    ...
    D3D12_ROOT_SIGNATURE_DESC1 rootSignatureDesc = {_countof(rootParameters),
...
bool D3D12GraphicsManager::InitializeShaders() {
    ...
    if (!m_pCommandList) {
        if (FAILED(hr = m_pDev->CreateCommandList(
                       0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_pCommandAllocator,
...
void D3D12GraphicsManager::InitializeBuffers(const Scene& scene) {
    ...
    for (auto _it : scene.GeometryNodes) {
        ...
        if (pGeometryNode && pGeometryNode->Visible()) {
            ...
            for (decltype(vertexPropertiesCount) i = 0;
                 i < vertexPropertiesCount; i++) {
                const SceneObjectVertexArray& v_property_array =
...
HRESULT D3D12GraphicsManager::PopulateCommandList() {
    ...
    // re-recording.
    if (FAILED(hr = m_pCommandList->Reset(m_pCommandAllocator,
                                          m_pPipelineState))) {
        return hr;
    }
    ...
    rtvHandle.ptr = m_pRtvHeap->GetCPUDescriptorHandleForHeapStart().ptr +
                    m_nFrameIndex * m_nRtvDescriptorSize;
    ...
    m_pCommandList->SetGraphicsRootDescriptorTable(
        1, m_pSamplerHeap->GetGPUDescriptorHandleForHeapStart());
    ...
    for (auto dbc : m_DrawBatchContext) {
        ...
        uint32_t                    nFrameResourceDescriptorOffset =
            m_nFrameIndex *
            (2 * kMaxSceneObjectCount);  // 2 descriptors for each draw call
        cbvSrvHandle.ptr =
            m_pCbvHeap->GetGPUDescriptorHandleForHeapStart().ptr +
            (nFrameResourceDescriptorOffset +
             i * 2 /* 2 descriptors for each batch */) *
                m_nCbvSrvDescriptorSize;
        ...
        for (uint32_t j = 0; j < dbc.property_count; j++) {
            m_pCommandList->IASetVertexBuffers(
                j, 1, &m_VertexBufferView[vertex_buffer_view_offset++]);
        }
        ...
        if (dbc.material) {
            if (auto texture = dbc.material->GetBaseColor().ValueMap) {
                ...
                srvHandle.ptr =
                    m_pCbvHeap->GetGPUDescriptorHandleForHeapStart().ptr +
                    (kTextureDescStartIndex + texture_index) *
                        m_nCbvSrvDescriptorSize;
...
bool D3D12GraphicsManager::SetPerBatchShaderParameters(int32_t index) {
    ...
    if (void* rigidBody = m_DrawBatchContext[index].node->RigidBody()) {
        // the geometry has rigid body bounded, we blend the simlation result
        // here.
    ...
    if (m_DrawBatchContext[index].material) {
        ...
        if (color.ValueMap) {
            pbc.specularColor = vec4(-1.0f);
        } else {
            pbc.specularColor = color.Value;
        }

        Parameter param =
            m_DrawBatchContext[index].material->GetSpecularPower();
        pbc.specularPower = param.Value;
    }

    memcpy(m_pCbvDataBegin +
               m_nFrameIndex *
                   kSizeConstantBufferPerFrame  // offset by frame index
