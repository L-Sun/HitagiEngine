...
inline ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags)
    : flags_(flags), inputNameBuf_(std::make_unique<std::array<char, INPUT_NAME_BUF_SIZE>>()) {
...
inline void ImGui::FileBrowser::Display() {
    ...
    std::array driveStr     = {currentDrive, ':', '\0'};

    PushItemWidth(4 * GetFontSize());
    if (BeginCombo("##select_drive", driveStr.data())) {
        ...
        for (int i = 0; i < 26; ++i) {
            ...
            std::array selectableStr = {driveCh, ':', '\0'};
            bool       selected      = currentDrive == driveCh;

            if (Selectable(selectableStr.data(), selected) && !selected) {
                std::array newPwd = {driveCh, ':', '\\', '\0'};
                SetPwd(newPwd.data());
            }
        }
    }
