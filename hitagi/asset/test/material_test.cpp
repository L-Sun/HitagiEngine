...
TEST(MaterialTest, InstanceDefaultValue) {
    ...
    EXPECT_VEC_EQ(instance->GetParameter<vec2f>("param2").value(), vec2f(1, 2));
...
TEST(MaterialTest, ParameterLayout) {
    ...
    EXPECT_VEC_EQ(*reinterpret_cast<vec2f*>(buffer.GetData() + 0), vec2f(1, 2));
    EXPECT_EQ(*reinterpret_cast<float*>(buffer.GetData() + 8), 1.0f);
    EXPECT_VEC_EQ(*reinterpret_cast<vec4f*>(buffer.GetData() + 16), vec4f(1, 2, 3, 4));
    EXPECT_VEC_EQ(*reinterpret_cast<vec2f*>(buffer.GetData() + 32), vec2f(1, 2));
    EXPECT_VEC_EQ(*reinterpret_cast<vec3f*>(buffer.GetData() + 48), vec3f(1, 2, 3));
}
